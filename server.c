    ...
    for (;;)
    {
        ...
        if (!strcmp(request[2], "REG"))
        {
            ...
            if (existing_client)
            {
                ...
                register_client(cache, request[3], current_client_address, current_client_port, current_time);
            if (heartbeat == '0')
                send_ack_ok(fd, client_addr, request[1][0]);
        }
        ...
        else if (!strcmp(request[2], "LOOKUP"))
        {
            if (find_client_by_address(cache, current_client_address, current_client_port))
            {
                ...
                if (!existing_client)
                    send_ack_notfound(fd, client_addr, request[1][0]);
                else
                    send_client_info(fd, client_addr, request[1][0], existing_client);
            }
        }
