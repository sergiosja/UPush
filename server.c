...
{
	int message_length = CLIENT_INFO_SIZE + strlen(current->nick) + strlen(current->address) + get_port_length(current->port);
	char *message = malloc(message_length);
	snprintf(message, message_length, "ACK %c NICK %s IP %s PORT %d", number, current->nick, current->address, current->port);
    int wc = send_packet(fd, message, strlen(message), 0, (struct sockaddr *) &address, sizeof(struct sockaddr_in));
    check_perror(wc, "send_packet");
    free(message);
}
